...
export default function ApplicationsPage() {
    ...
    const handleResumeApp = async (formType: string, appId: number) => {
        try {
            setIsResuming(true);
            const data = await api.initSession(formType, appId);
            if (data.session_id) {
                localStorage.setItem(`session_${data.session_id}_reqs`, JSON.stringify(data.required_documents));
                localStorage.setItem(`session_${data.session_id}_form`, formType);
                router.push(`/session/${data.session_id}/upload`);
            }
        } catch (e) {
            console.error("Failed to resume application:", e);
            alert("Failed to resume application. Please try again.");
            setIsResuming(false);
        }
    };
                        ...
                        <tbody className="divide-y divide-gray-100 dark:divide-gray-700">
                            {stats?.recent_activities?.length > 0 ? (
                                stats.recent_activities.map((act: any) => (
                                    <tr
                                        key={act.id}
                                        className={`transition-colors ${act.status === 'in_progress' ? 'cursor-pointer hover:bg-blue-50 dark:hover:bg-blue-900/20' : 'hover:bg-gray-50 dark:hover:bg-gray-700/50'}`}
                                        onClick={() => {
                                            if (act.status === 'in_progress' && !isResuming) {
                                                handleResumeApp(act.form_type, act.id);
                                            }
                                        }}
                                    >
                                            ...
                                            <div className="flex items-center justify-end gap-2">
                                                {act.status === 'in_progress' && (
                                                    <button
                                                        onClick={(e) => {
                                                            e.stopPropagation();
                                                            if (!isResuming) handleResumeApp(act.form_type, act.id);
                                                        }}
                                                        disabled={isResuming}
                                                        className="text-sm bg-blue-100 text-blue-700 hover:bg-blue-200 dark:bg-blue-900/30 dark:text-blue-300 dark:hover:bg-blue-800/50 px-3 py-1 rounded-md font-medium transition-colors disabled:opacity-50"
                                                    >
