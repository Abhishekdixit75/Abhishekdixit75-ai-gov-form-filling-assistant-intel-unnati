...
async def init_session(
    ...
    import re
    if not re.match(r"^[a-zA-Z0-9_]+$", form_type):
        ...
        try:
            ...
            if email:
                ...
                if user:
                    ...
                    # --- NEW: Create or Resume Application Record ---
                    if app_id and app_id != "null" and app_id != "undefined":
                        try:
                            # Resume existing application
                            app_record = db.query(models.Application).filter(
                                models.Application.id == int(app_id),
                                models.Application.user_id == user.id
                            ).first()
                            
                            if app_record:
                                logger.info(f"  ✓ Resuming existing application APP-{app_record.id}")
                                store.app_id = app_record.id
                            else:
                                logger.warning(f"  ⚠ Application {app_id} not found or unauthorized. Creating new.")
                        ...
                        db.commit()
                        db.refresh(new_app)
                        logger.info(f"  ✓ Created new application APP-{new_app.id}")
                        store.app_id = new_app.id
                    # --------------------------------------
        except Exception as e:
            logger.warning(f"Failed to auto-fill profile or handle application: {e}")
    # -------------------------------------
...
async def finalize_session(
    ...
    # --- Clean up uploaded files to protect privacy ---
    try:
        session_dir = BASE_DATA_DIR / session_id
        if session_dir.exists():
            shutil.rmtree(session_dir)
            logger.info(f"  ✓ Cleaned up uploaded data for session {session_id[:8]}")
    except Exception as e:
        logger.error(f"  ✗ Failed to clean up session data: {e}")
    # ------------------------------------------
...
async def upload_and_process(
    ...
    import re
    if not re.match(r"^[a-zA-Z0-9_]+$", form_type):
        return JSONResponse({"error": "Invalid form type format."}, status_code=400)
